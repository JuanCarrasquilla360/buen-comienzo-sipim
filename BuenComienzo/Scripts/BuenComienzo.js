...
function LoadCalendarScript(callback) {
    function LoadFullCalendarScript() {
        if (!$.fn.fullCalendar) {
            $.getScript(ResolveClientURL('plugins/fullcalendar/fullcalendar.js'), callback);
        }
        else {
    ...
    if (!$.fn.moment) {
        $.getScript(ResolveClientURL('plugins/moment/moment.min.js'), LoadFullCalendarScript);
    }
    else {
...
function LoadLeafletScript(callback) {
    if (!$.fn.L) {
        $.getScript(ResolveClientURL('plugins/leaflet/leaflet.js'), callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadTimePickerScript(callback) {
    if (!$.fn.timepicker) {
        $.getScript(ResolveClientURL('plugins/jquery-ui-timepicker-addon/jquery-ui-timepicker-addon.min.js'), callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadBootstrapValidatorScript(callback) {
    if (!$.fn.bootstrapValidator) {
        $.getScript(ResolveClientURL('plugins/bootstrapvalidator/bootstrapValidator.min.js'), callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadSelect2Script(callback) {
    if (!$.fn.select2) {
        $.getScript(ResolveClientURL('plugins/select2/select2.min.js'), callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadDataTablesScripts(callback) {
    function LoadDatatables() {
        $.getScript(ResolveClientURL('plugins/datatables/jquery.dataTables.js'), function () {
            $.getScript(ResolveClientURL('plugins/datatables/ZeroClipboard.js'), function () {
                $.getScript(ResolveClientURL('plugins/datatables/TableTools.js'), function () {
                    $.getScript(ResolveClientURL('plugins/datatables/dataTables.bootstrap.js'), callback);
                });
            });
        });
    }
...
function LoadFineUploader(callback) {
    if (!$.fn.fineuploader) {
        $.getScript(ResolveClientURL('plugins/fineuploader/jquery.fineuploader-5.0.5.min.js'), callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadXChartScript(callback) {
    function LoadXChart() {
        $.getScript(ResolveClientURL('plugins/xcharts/xcharts.min.js'), callback);
    }
    function LoadD3Script() {
        if (!$.fn.d3) {
            $.getScript(ResolveClientURL('plugins/d3/d3.min.js'), LoadXChart)
        }
        else {
            LoadXChart();
        }
...
function LoadFlotScripts(callback) {
    function LoadFlotScript() {
        $.getScript(ResolveClientURL('plugins/flot/jquery.flot.js'), LoadFlotResizeScript);
    }
    function LoadFlotResizeScript() {
        $.getScript(ResolveClientURL('plugins/flot/jquery.flot.resize.js'), LoadFlotTimeScript);
    }
    function LoadFlotTimeScript() {
        $.getScript(ResolveClientURL('plugins/flot/jquery.flot.time.js'), callback);
    }
    if (!$.fn.flot) {
        LoadFlotScript();
    }
...
function LoadMorrisScripts(callback) {
    function LoadMorrisScript() {
        if (!$.fn.Morris) {
            $.getScript(ResolveClientURL('plugins/morris/morris.min.js'), callback);
        }
        else {
            if (callback && typeof (callback) === "function") {
                callback();
    ...
    if (!$.fn.raphael) {
        $.getScript(ResolveClientURL('plugins/raphael/raphael-min.js'), LoadMorrisScript);
    }
    else {
        LoadMorrisScript();
    }
...
function LoadAmchartsScripts(callback) {
    function LoadAmchartsScript() {
        $.getScript(ResolveClientURL('plugins/amcharts/amcharts.js'), LoadFunnelScript);
    }
    function LoadFunnelScript() {
        $.getScript(ResolveClientURL('plugins/amcharts/funnel.js'), LoadSerialScript);
    }
    function LoadSerialScript() {
        $.getScript(ResolveClientURL('plugins/amcharts/serial.js'), LoadPieScript);
    }
    function LoadPieScript() {
        $.getScript(ResolveClientURL('plugins/amcharts/pie.js'), callback);
    }
    if (!$.fn.AmCharts) {
        LoadAmchartsScript();
    }
...
function LoadChartistScripts(callback) {
    function LoadChartistScript() {
        $.getScript(ResolveClientURL('plugins/chartist/chartist.min.js'), callback);
    }
    if (!$.fn.Chartist) {
        LoadChartistScript();
    }
...
function LoadSpringyScripts(callback) {
    function LoadSpringyScript() {
        $.getScript(ResolveClientURL('plugins/springy/springy.js'), LoadSpringyUIScript);
    }
    function LoadSpringyUIScript() {
        $.getScript(ResolveClientURL('plugins/springy/springyui.js'), callback);
    }
    if (!$.fn.Springy) {
        LoadSpringyScript();
    }
...
function LoadFancyboxScript(callback) {
    if (!$.fn.fancybox) {
        $.getScript(ResolveClientURL('plugins/fancybox/jquery.fancybox.js'), callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadKnobScripts(callback) {
    if (!$.fn.knob) {
        $.getScript(ResolveClientURL('plugins/jQuery-Knob/jquery.knob.js'), callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadSparkLineScript(callback) {
    if (!$.fn.sparkline) {
        $.getScript(ResolveClientURL('plugins/sparkline/jquery.sparkline.min.js'), callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
//
//
function OpenModalBox(header, inner, bottom, tamano) {
    var modalbox = $('#modalbox', window.parent.document);

    modalbox.find('.modal-header-name span').html(header);
    modalbox.find('.devoops-modal-header').css('background-image', 'linear-gradient(rgb(232,232,232), rgb(224,224,224))');
    modalbox.find('.devoops-modal-header').css('border', '1px solid #ccc');
    modalbox.find('.devoops-modal-inner').html(inner);
    modalbox.find('.devoops-modal-inner').attr('onclick', '');
    modalbox.find('.devoops-modal-inner').css('cursor', 'default');
    modalbox.find('.devoops-modal-bottom').html(bottom);
    modalbox.find('.devoops-modal-bottom').css('background-color', "#eaeaea");
    modalbox.find('.devoops-modal').css('width', tamano);
    modalbox.find('.devoops-modal').css('border', '1px solid #ccc');
    modalbox.fadeIn('fast');

    $('body', window.parent.document).addClass("body-expanded");
}

function OpenModalBoxMsg(header, inner, bottom, color, top, content) {
    var modalbox;

    if (content != null)
        modalbox = $('#modalboxFrame', content);
    else
        modalbox = $('#modalboxFrame');

    modalbox.find('.modal-header-name span').html(header);
    modalbox.find('.devoops-modal-header').css('background-color', color);
    modalbox.find('.devoops-modal-header').css('background-image', 'none');
    modalbox.find('.devoops-modal-header').css('border', '1px solid ' + color);
    modalbox.find('.devoops-modal-inner').html(inner);
    modalbox.find('.devoops-modal-inner').attr('onclick', 'CloseModalBoxMsg()');
    modalbox.find('.devoops-modal-inner').css('cursor', 'pointer');
    modalbox.find('.devoops-modal-bottom').html(bottom);
    modalbox.find('.devoops-modal-bottom').css('background-color', color);
    modalbox.find('.devoops-modal').css('width', '500');
    modalbox.find('.devoops-modal').css('border', '1px solid ' + color);
    modalbox.find('.devoops-modal').css('top', top + 'px');
    modalbox.fadeIn('fast');
    $('body', window.parent.document).addClass("body-expanded");
}
//
...
function CloseModalBox() {
    console.log('CloseModelBox', 'BuenComienzos.js');
    var modalbox = $('#modalbox', window.parent.document);
    modalbox.fadeOut('fast', function () {
        modalbox.find('.modal-header-name span').children().remove();
        modalbox.find('.devoops-modal-inner').children().remove();
        modalbox.find('.devoops-modal-bottom').children().remove();
        $('body', window.parent.document).removeClass("body-expanded");
    });
}

function CloseModalBoxMsg(content) {
    var modalbox;
...
function ConfigurarGrid(tabla, paginacion, ordenar, scrollGridX, scrollGridY) {

    if (scrollGridX != null || scrollGridY != null) {
        return $('#' + tabla).dataTable({
            "destroy": true,
            "scrollY": (scrollGridY != null) ? scrollGridY : '100%',
            "scrollX": (scrollGridX != null) ? scrollGridX : null,
            "aaSorting": [[1, "asc"]],
            "aoColumnDefs": [{ "bSortable": false, "aTargets": [0] }],
            "sDom": "<<f><'col-sm-6 text-right'l><'clearfix'>>rt<<i><'col-sm-12 text-right'p><'clearfix'>>",
            //"sDom": 'T<"clear">lfrtip',
            "sPaginationType": "bootstrap",
            "searching": true,
            "bFilter": true,
            "bSort": (ordenar != null) ? ordenar : true,
            "bLengthChange": false,
            "iDisplayLength": (paginacion != null) ? paginacion : 10,
            "oLanguage": {
                "sZeroRecords": "No hay registros para mostrar",
                "sInfoFiltered": "(filtrado de _MAX_ registros totales)",
                "sSearch": "Filtrar: ",
                "sSearchPlaceholder": "Texto a filtrar",
                "oPaginate": {
                    "sNext": "",
                    "sPrevious": ""
                },
                "sInfo": "Un total de _TOTAL_ registros a mostrar (_START_ hasta _END_)",
            ...
            "aButtons": [
                {
                    ...
                    "sButtonText": "Copiar",
                    "sInfo": "Tabla copiada al portapapeles",
                    "mColumns": "sortable"
                },
                {
                    "sExtends": "print",
                ...
                },
                {
                    ...
                    "aButtons": [
                        {
                            "sExtends": "csv",
                            "sTitle": "<%=gvUsuarios.ToolTip%>",
                            "sToolTip": "Exportar como CSV",
                            "sFileName": "<%=gvUsuarios.ToolTip%>.csv",
                            "mColumns": "sortable"
                        },
                        {
                            "sExtends": "xls",
                            "sTitle": "<%=gvUsuarios.ToolTip%>",
                            "sToolTip": "Exportar como Excel",
                            "sFileName": "<%=gvUsuarios.ToolTip%>.xls",
                            "mColumns": "sortable"
                        },
                        {
                            "sExtends": "pdf",
                            ...
                            "sFileName": "<%=gvUsuarios.ToolTip%>.pdf",
                            "mColumns": "sortable"
                        }
                    ]
                }
...
$(document).ready(function () {
    ...
    $('#mainMenu').css('min-height', height)
    ...
    $('body').on('click', 'a.close-link', function (e) {
        e.preventDefault();
        if ($('#modalboxFrame').find('.devoops-modal-inner').children().length > 0)
            CloseModalBoxMsg();
        else {
            if ($('#modalbox').find('.devoops-modal-inner').children().length > 0)
                CloseModalBox();
            else {
                if ($('#modalboxFrame', $("#framePrincipal").contents()).find('.devoops-modal-inner').children().length > 0)
                    CloseModalBoxMsg($("#framePrincipal").contents());
            }
        }
        //CloseModalBox();
    });
...
function AbrirVentanaPopup(header, pagina, botones, tamano, esconder) {
    $.ajax({
        mimeType: 'text/html; charset=utf-8', // ! Need set mimeType only when run from local file
        url: pagina,
        type: 'GET',
        success: function (data) {
            OpenModalBox(header, data, botones, tamano + 'px');
            if (esconder != null && esconder)
                $('#modalbox', window.parent.document).hide();
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert('Ha ocurrido un error al tratar de abrir la página, por favor contacte al administrador del sistema.');
        },
        dataType: "html",
        async: false
    });
...
function Mensaje(mensaje, tipoMensaje, top) {
    var botones = $('<div class="text-center"><a href="#" class="btn btn-info" style="margin-bottom: 0px;" onclick="CloseModalBoxMsg();">Aceptar</a></div>');
    var titulo = '';
    var cuerpo = '';
    var color = '#eaeaea';
    switch (tipoMensaje) {
        case 'Error':
            ...
            cuerpo = '<div class="row show-grid-forms"><div class="col-sm-2"><img src="' + ResolveClientURL('img/Error.png') + '" alt="" width="60px" /></div><div class="col-sm-10"><label class="control-label" style="cursor:pointer">' + mensaje + '</label></div></div>';
            break;
        case 'Exito':
            titulo = 'La operación se realizó exitosamente.';
            cuerpo = '<div class="row show-grid-forms"><div class="col-sm-2"><img src="' + ResolveClientURL('img/Exito.png') + '" alt="" width="60px" /></div><div class="col-sm-10"><label class="control-label" style="cursor:pointer">' + mensaje + '</label></div></div>';
            break;
        case 'Advertencia':
            titulo = 'Advertencia';
            cuerpo = '<div class="row show-grid-forms"><div class="col-sm-2"><img src="' + ResolveClientURL('img/Advertencia.png') + '" alt="" width="60px" /></div><div class="col-sm-10"><label class="control-label" style="cursor:pointer">' + mensaje + '</label></div></div>';
            break;
        case 'Pregunta':
            //botones = $('<div class="text-center"><a href="#" class="btn btn-info" onclick="CloseModalBox();">Aceptar</a></div>');
            break;
        default:
            ""
    }

    OpenModalBoxMsg(titulo, cuerpo, '', color, top);
}

function MensajeSitio(mensaje, tipoMensaje, top) {
    var botones = $('<div class="text-center"><a href="#" class="btn btn-info" style="margin-bottom: 0px;" onclick="CloseModalBoxMsg();">Aceptar</a></div>');
    var titulo = '';
    var cuerpo = '';
    var color = '#eaeaea';
    ...
    OpenModalBoxMsg(titulo, cuerpo, '', color, top, window.parent.document);
}

function habilitarBotones() {
    $('#btnGuardar', window.parent.document).removeAttr('disabled');
    $('#btnCancelar', window.parent.document).removeAttr('disabled');
}
var temD;
function SeleccionFecha(campoFecha, cambiarAno, cambiarMes, rango, minDate, maxDate) {
    var rangoAnos = '';
    jQuery(function ($) {
    ...
    $('#' + campoFecha).css('position', 'relative').css('z-index', '100000');
    $('#' + campoFecha).datepicker({
        ...
        changeYear: cambiarAno,
        changeMonth: cambiarMes,
        ...
        onSelect: function (selected) {
            selected = $.datepicker.parseDate('dd/mm/yy', selected);
            if ($(this).attr("minDate") != null) {
                selected.setDate(selected.getDate() + 1);
                var cFechas = $(this).attr("minDate").split(',');

                for (var i = 0; i < cFechas.length; i++)
                    $("#" + cFechas[i]).datepicker("option", "minDate", selected)

            }
            if ($(this).attr("minEqualDate") != null) {
                selected.setDate(selected.getDate());
                var cFechas = $(this).attr("minEqualDate").split(',');

                for (var i = 0; i < cFechas.length; i++)
                    $("#" + cFechas[i]).datepicker("option", "minDate", selected)
            }
            if ($(this).attr("maxEqualDate") != null) {
                selected.setDate(selected.getDate());
                var cFechas = $(this).attr("maxEqualDate").split(',');

                for (var i = 0; i < cFechas.length; i++)
                    $("#" + cFechas[i]).datepicker("option", "maxDate", selected)
            }
            if ($(this).attr("maxDate") != null) {
                selected.setDate(selected.getDate() - 1);
                var cFechas = $(this).attr("maxDate").split(',');
        ...
        beforeShow: function () {
            ...
            var estiloAno = "select-personalizado";
            var estiloMes = "select-personalizado";

            $('#ui-datepicker-div').unbind('beforeFadeIn');
            ...
            $('#ui-datepicker-div').bind('beforeFadeIn', function () {
                ...
                if (!month.hasClass(estiloMes)) {
                    if (!cambiarMes)
                        month.css('color', 'white');
                    else
                        month.css('color', 'black');
                }
                if (!year.hasClass(estiloAno)) {
...
function OrdenarLista(nombreLista) {
    var opciones = $('#' + nombreLista + ' option');
    var seleccionado = $('#' + nombreLista).val();
    ...
    $('#' + nombreLista).empty().append(opciones);
    $('#' + nombreLista).val(seleccionado);
}

function ValidarCampos(campos, Nombreform, form) {
    var scriptText = "LoadBootstrapValidatorScript(Validaciones);\n";

    if (form == null)
        form = window.parent.document;

    if (Nombreform == null)
        Nombreform = "frmPopupNuevo";

    scriptText += "function Validaciones() {\n" +
                                    "$('#" + Nombreform + "').bootstrapValidator({\n" +
                                    "excluded: [':disabled'],\n" +
                                    "fields: {\n";

    for (var i = 0; i < campos.length; i++) {

        if ($('#' + campos[i], form).attr("type") == "radio") {
            scriptText += $('#' + campos[i], form).attr("name") + ": {\n" +
                    "group: '.group',\n" +
                    "validators: {\n";
        }
        else {
            scriptText += $('#' + campos[i], form).attr("id") + ": {\n" +
                    "group: '.group',\n" +
                    "validators: {\n";
        }

        if ($('#' + campos[i], form).attr("maxlength") != null) {
            scriptText += "stringLength: {" +
                            "max: " + $('#' + campos[i], form).attr("maxlength") + "," +
                            "message: 'El campo debe ser menor que " + $('#' + campos[i], form).attr("maxlength") + " caracteres de longitud'" +
                        "},\n";
        }

        if ($('#' + campos[i], form).attr("vacio") == "n") {

            scriptText += "notEmpty: {\n" +
                                "message: 'Este campo es requerido'\n" +
                            "},\n";
        }
        if ($('#' + campos[i], form).attr("numerico") == "s") {
            scriptText += "digits: {\n" +
                                "message: 'El campo sólo acepta valores numéricos'\n" +
                            "},\n";

            AddScriptParent("$('#" + campos[i] + "').inputmask('9', {\n" +
                                "repeat: '" + $('#' + campos[i], form).attr("maxlength") + "',\n" +
                                "greedy: false\n" +
                            "});\n");
        }

        if ($('#' + campos[i], form).attr("fecha") == "s") {
            scriptText += "date: {\n" +
            ...
            AddScriptParent("$('#" + $('#' + campos[i], form).attr("id") + "').datepicker().on('change', function () {\n" +
                                    "if($(this).val() != '') {\n" +
                                        "var fecha = $.datepicker.parseDate('dd/mm/yy', $(this).val());\n" +
                                        "if ($(this).attr('minDate') != null) {\n" +
                                            "fecha.setDate(fecha.getDate() + 1);\n" +
                                            "var cFechas = $(this).attr('minDate').split(',');\n" +
                                            "for (var i = 0; i < cFechas.length; i++)\n" +
                                            "{\n" +
                                                "$('#' + cFechas[i]).datepicker('option', 'minDate', fecha)\n" +
                                                "$('#' + cFechas[i]).change();\n" +
                                            "}\n" +
                                        "}\n" +
                                        "fecha = $.datepicker.parseDate('dd/mm/yy', $(this).val());\n" +
                                        "if ($(this).attr('minEqualDate') != null) {\n" +
                                        ...
                                        "if ($(this).attr('maxEqualDate') != null) {\n" +
                                            ...
                                            "for (var i = 0; i < cFechas.length; i++)\n" +
                                            "{\n" +
                                                "$('#' + cFechas[i]).datepicker('option', 'maxDate', fecha)\n" +
                                                "$('#' + cFechas[i]).change();\n" +
                                            "}\n" +
                                        "}\n" +
                                        "$('#" + Nombreform + "').data('bootstrapValidator').revalidateField('" + $('#' + campos[i], form).attr("id") + "');" +
                                    "}\n" +
                                "});\n");
        }

        if ($('#' + campos[i], form).attr("hora") == "s") {
            scriptText += "regexp: {\n" +
                            "regexp: /^(0[0-9]{1}|1[0-9]{1}|2[0-3]{1}):[0-5]{1}[0-9]{1}$/,\n" +
                            "message: 'El valor ingresado no es válido'\n" +
                        "},\n";

            AddScriptParent("$('#" + campos[i] + "').inputmask({\n" +
                                "alias: 'datetime',\n" +
                                "mask: 'h:s',\n" +
                                "placeholder: '__:__',\n" +
                                "hourFormat: '24'\n" +
        ...
        if ($('#' + campos[i], form).attr("celular") == "s") {
            scriptText += "regexp: {\n" +
                            "regexp: /^3\\d{9,9}$/,\n" +
        ...
        if ($('#' + campos[i], form).attr("telefono") == "s") {
            scriptText += "regexp: {\n" +
                            "regexp: /^\\d{7,7}$/,\n" +
        ...
        if ($('#' + campos[i], form).attr("email") == "s") {

            scriptText += "emailAddress: {\n" +
        ...
        if ($('#' + campos[i], form).attr("identico") != null) {
            scriptText += "identical: {\n" +
                            "field: '" + $('#' + campos[i], form).attr("identico") + "',\n" +
                            "message: 'La contraseña y su confirmación no son identifcos'\n" +
                        "},\n";
        }

        if ($('#' + campos[i], form).attr("moneda") != null) {
            scriptText += "regexp: {\n" +
                            "regexp: /^\\$((\\d+)|(\\d{1,3})(\\,\\d{3}|)*)(\\.\\d{2}|)$/,\n" +
                            "message: 'El valor ingresado no es válido'\n" +
                        "},\n";

            AddScriptParent("$('#" + campos[i] + "').inputmask('decimal', {\n" +
                                "radixPoint: '.',\n" +
                                "groupSeparator: ',',\n" +
                                "digits: 2,\n" +
                                "autoGroup: true,\n" +
        ...
        if ($('#' + campos[i], form).attr("decimal") != null) {
            scriptText += "regexp: {\n" +
                            "regexp: /^(\\d+)(\\.\\d{2}|)$/,\n" +
                            "message: 'El valor ingresado no es válido'\n" +
                        "},\n";

            AddScriptParent("$('#" + campos[i] + "').inputmask('Regex', {\n" +
                                "regex: '[0-9]*\\\\.[0-9]{2}'\n" +
                            "});\n");
        }

        if ($('#' + campos[i], form).attr("letras") != null) {
            scriptText += "regexp: {\n" +
                            "regexp: /^((?!NO TIENE|SIN DATO|NOTIENE|^NA$|N\\/A|N\\.A|XX|NINGUNO|no tiene|notiene|sin dato|^na$|n\\/a|n\\.a|xx|ninguno).{3,})*$/,\n" +
                            "message: 'El valor ingresado no es válido'\n" +
                        "},\n";

            AddScriptParent("$('#" + campos[i] + "').inputmask('Regex', {\n" +
                                "regex: '^[a-zA-Z_áéíóúñÁÉÍÓÚÑ\\\\s]*$'\n" +
                            "});\n");
        }

        if ($('#' + campos[i], form).attr("nroDoc") != null) {
            scriptText += "regexp: {\n" +
                            "regexp: /^[0-9a-zA-Z]+$/,\n" +
                            "message: 'El valor ingresado no es válido'\n" +
                        "},\n";
        ...
        if ($('#' + campos[i], form).attr("query") == null) {
            AddScriptParent("$('#" + campos[i] + "').keypress(function () {\n" +
                                "var regex = /[<>'\"/+\\-\\_{}()\\[\\]\\^¡?¿\\\=*~¨´`%&$#!|°¬]/\n" +
        ...
        AddScriptParent("$('#" + campos[i] + "').keyup(function () {\n" +
                            "var _event = event || window.event;\n" +
                            "var key = _event.keyCode || _event.which;\n" +
                            "if((key == 8 || key == 46) && $(this).val().length == 0) {\n" +
                                "$('#" + Nombreform + "').data('bootstrapValidator').revalidateField('" + campos[i] + "');" +
                            "}\n" +
                        "});");

        if ($('#' + campos[i], form).attr("maxDateI") != null) {
            scriptText += "callback: {\n" +
                                "message: '" + $('#' + campos[i], form).attr("mensaje") + "',\n" +
                                "callback: function(value, validator) {\n" +
                                    "var m = new moment(value, 'DD/MM/YYYY', true);\n" +
                                    "if (!m.isValid())\n" +
                                        "return true;\n" +
                                    "m2 = moment($('#" + $('#' + campos[i], form).attr("maxDateI") + "').val(), 'DD/MM/YYYY', true);\n" +
                                    "if (!m2.isValid())\n" +
                                        "return true;\n" +
                                    "return m.isBefore(m2);\n" +
                                "}\n" +
                            "},\n";

        }

        if ($('#' + campos[i], form).attr("maxEqualDateI") != null) {
            scriptText += "callback: {\n" +
                                "message: '" + $('#' + campos[i], form).attr("mensaje") + "',\n" +
                                "callback: function(value, validator) {\n" +
                                    "var m = new moment(value, 'DD/MM/YYYY', true);\n" +
                                    "if (!m.isValid())\n" +
                                        "return true;\n" +
                                    "m2 = moment($('#" + $('#' + campos[i], form).attr("maxEqualDateI") + "').val(), 'DD/MM/YYYY', true);\n" +
                                    "if (!m2.isValid())\n" +
                                        "return true;\n" +
                                    "return m.isBefore(m2.add(1, 'days'));\n" +
                                "}\n" +
                            "},\n";
    ...
    scriptText += "}\n" +
                    ...
                    "if (typeof posValidaciones == 'function')\n" +
                        "posValidaciones();" +
                "}";

    AddScriptParent(scriptText, form);
}

function AddScriptParent(scriptText, form) {

    if (form == null)
        form = window.parent.document;

    var script = form.createElement("script");
    script.type = "text/javascript";
    script.text = scriptText;
    script.id = "scriptsPopup";
    form.body.appendChild(script);
}

function Removerscripts(form) {
    window.parent.preGuardado = null;
    window.parent.posValidaciones = null;
    $('[id*=scriptsPopup]', form).remove();
}

function llenarListaAjax(lista, nombLista, form, defaultValue, ejecutarChange, campFiltro, valFiltro) {

    if (valFiltro == null)
        valFiltro = "";

    var datos = retornarDatosCache(nombLista);

    if (form != null) {

        if (datos && campFiltro == null && valFiltro == '') {
            setSelectOptions($('#' + lista, form), datos, "Codigo", "Descripcion", defaultValue);
            if (ejecutarChange) {
                $('#' + lista, form).change();
            }
        }
        else {
            ejecutarMetodo("../WebMethods.aspx/ConsultarLista", "nombreLista,campoFiltro,valorfiltro", nombLista + "," + campFiltro + "," + valFiltro.toString(),
            function (resp) {
                var data = JSON.parse(resp.d);
                almacenarCache(nombLista, data);
                setSelectOptions($('#' + lista, form), data, "Codigo", "Descripcion", defaultValue);
                if (ejecutarChange) {
                    $('#' + lista, form).change();
                }
            });
        ...
        if (datos && campFiltro == null && valFiltro == '') {
            setSelectOptions($('#' + lista), datos, "Codigo", "Descripcion", defaultValue);
            if (ejecutarChange) {
                $('#' + lista).change();
            }
        }
        else {
            ejecutarMetodo("./Paginas/WebMethods.aspx/ConsultarLista", "nombreLista,campoFiltro,valorfiltro", nombLista + "," + campFiltro + "," + valFiltro,
            function (resp) {
                var data = JSON.parse(resp.d);
                setSelectOptions($('#' + lista), data, "Codigo", "Descripcion", defaultValue);
                if (ejecutarChange)
                    $('#' + lista).change();
            });
        }
...
function ejecutarMetodo(metodo, campos, valores, funcSuccess) {
    campos = campos.split(',');
...
function ejecutarMetodoAsync(metodo, campos, valores, funcSuccess) {
    campos = campos.split(',');
    valores = valores.split(',');
...
function ConfigurarPanel(panel, form, cerrarPaneles) {
    ...
    $('#' + panel, form).css('min-height', height)
		.on('click', '.collapse-link', function (e) {
		            ...
		            if (panel != $(this).attr('id')) {
		                $(this).find('div.box').each(function () {
		                    if ($(this).find('i.fa-chevron-up').length > 0) {
		                        $(this).find('div.box-content').slideToggle('fast');
		    ...
		    var box = $(this).closest('div.box', form);
		    var button = $(this).find('i', form);
		    var content = box.find('div.box-content', form);
...
function MensajeLateral(mensaje) {

    $('#pMensaje', window.parent.document).html(mensaje);
    $('#dvMensajeExito', window.parent.document).toggle('slide', { direction: 'Right' }, 300);
    setTimeout(function () { $('#dvMensajeExito', window.parent.document).toggle('slide', { direction: 'Right' }, 300); }, 1500);
}
...
function editarDireccion() {
    ...
    var arrayDirreccionEditada = tv1 + espacioNv1 + nV1.toUpperCase() + ap1 + espacioCa1 + ca1 + ((ca1 == "") ? " " : "") + signoNumero + nV2.toUpperCase() + ap2 + guion + pl.toUpperCase();
...
var datosBasicosCache = ['TipoCabecera', 'MotivoSinDocumento', 'Paises', 'ComunidadesNegras', 'RromgitanoKumpania', 'SistemaGeneralSeguridadSocial', 'Epss', 'TipoDiscapacidad', 'TipoVivienda', 'TipoFamilia', 'Parentesco', 'Ocupacion', 'NivelEducativo', 'EstadoCivil', 'Parentesco', 'TipoParticipante', 'ZonaUbicacion', 'GrupoEtnia', 'SistemaGeneralSeguridadSocial']

function cargarDatosBasicosCache(loaderAnimation) {
    
    var valorCarga = 100 / datosBasicosCache.length
    var porcentaje = 0;
    var count = 0;
    
    $.each(datosBasicosCache, function (i, value) {
        ejecutarMetodoAsync("./Paginas/WebMethods.aspx/ConsultarLista", "nombreLista,campoFiltro,valorfiltro", value + ",,",
        function (resp) {
            var datos = JSON.parse(resp.d);
            almacenarCache(value, datos);

            porcentaje += valorCarga;
            loaderAnimation.update(porcentaje);
            count++;
            
            if(count == datosBasicosCache.length)
                loaderAnimation.update(100);
        });
    });
}

function retornarDatosCache(id) {
    var cache = kizzy('DatosBasicosBCA');
    var datosCache = cache.get(id);
...
function almacenarCache(id, datos) {

    if (datosBasicosCache.indexOf(id) < 0)
        return;
    ...
    if (datosCache) {
        if (JSON.stringify(datos) != JSON.stringify(datosCache)) {
            cache.set(id, datos);
            datosCache = datos;
        }
    } else {
