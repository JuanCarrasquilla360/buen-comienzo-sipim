<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Visita.aspx.cs" Inherits="BuenComienzo.Paginas.Operacion.Visita" %>
    ...
    <form id="Visita" runat="server">
            ...
            <section class="content">
                ...
                <div class="row">
                    <div class="box">
                        ...
                        <!-- /.box-header -->
                        <div class="box-body">
                            <table id="grdVisita" class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        <%--<th>Nro. de cédula</th>
        ...
        <script type="text/javascript">
            ...
            function cargarDatos() {
                this.tabla = $("#grdVisita").DataTable({
                            "destroy": true,
                            "paging": true,
                            ...
                            'aoColumns': columnasJSON(['Código Visita', 'Nro de Documento', 'Nombres y apellidos', 'Documento Cabeza de Hogar', 'Nombre Cabeza de Hogar', 'Fecha Visita', 'Programa', 'Efectiva', 'Profesional', 'Perfil', '']),
                            "bServerSide": true,
                            "sAjaxSource": "../WebMethods.aspx/ConsultarVisita",
            ...
            function Editar(idVisita, nuevo) {                
                openPopup({
                    'title': 'Editar Cabeza de Hogar',
                    'width': '900',
                    'url': './Popups/EditarVisita.aspx?idVisita=' + idVisita,
                    'buttons': [{
                        'name': 'Guardar',
                        'click': function () {
                            if ($('#EditarVisita', parent.document).isValid()) {
                                var objVisita = new Object();
                                objVisita.idVisita = idVisita;
                                objVisita.nuevo = nuevo;
                                objVisita.documentoCabezaHogar = $('#txtDocumentoCabezaHogar', parent.document).val();
                                objVisita.tipoDocumento = $('#ddlTipoDocumento option:selected', parent.document).val();
                                objVisita.idDocumento = $('#txtDocumento', parent.document).val();
                                objVisita.primerNombre = $('#txtPrimerNombre', parent.document).val();
                                objVisita.segundoNombre = $('#txtSegundoNombre', parent.document).val();
                                objVisita.primerApellido = $('#txtPrimerApellido', parent.document).val();
                                objVisita.segundoApellido = $('#txtSegundoApellido', parent.document).val();
                                objVisita.fechaNacimiento = $('#txtFechaNacimiento', parent.document).val();
                                objVisita.sexo = $('#ddlSexo option:selected', parent.document).val();
                                objVisita.celular = $('#txtCelular', parent.document).val();
                                objVisita.telefono = $('#txtTelefono', parent.document).val();
                                objVisita.fechaVisita = $('#txtFechaVisita', parent.document).val();
                                objVisita.programa = $('#ddlPrograma option:selected', parent.document).val();
                                objVisita.actividadVisita = $('#ddlActividadVisita option:selected', parent.document).val();
                                objVisita.efectiva = $('#ddlEfectiva option:selected', parent.document).val();
                                objVisita.enfermeria = $('#ddlEnfermeria option:selected', parent.document).val();
                                objVisita.nutricion = $('#ddlNutricion option:selected', parent.document).val();
                                objVisita.saludBucal = $('#ddlSaludBucal option:selected', parent.document).val();
                                objVisita.saludAmbiental = $('#ddlSaludAmbiental option:selected', parent.document).val();
                                objVisita.saludMental = $('#ddlSaludMental option:selected', parent.document).val();
                                objVisita.spa = $('#ddlSpa option:selected', parent.document).val();
                                objVisita.areaSocial = $('#ddlAreaSocial option:selected', parent.document).val();
                                objVisita.observaciones = $('#txtObservaciones', parent.document).val();
                                

                                execMethod('../WebMethods.aspx/ActualizarVisita', 'POST', objVisita, function (resp) {
                                    try {
                ...
                parent.configChangeInput('txtDocumentoCabezaHogar', function () {
                    if ($('#txtDocumentoCabezaHogar', parent.document).val() !== '') {
                        var objDoc = new Object();
                        objDoc.idDocumento = $('#txtDocumentoCabezaHogar', parent.document).val();
                        execMethod('../WebMethods.aspx/ConsultarCabezaHogar', 'POST', objDoc, function (resp) {
                            var objResp = resp.d;
                            var objCabezaHogar = resp.d;
                            try {
                                objCabezaHogar = resp.d;
                                if (objCabezaHogar != null) {
                                    $('#txtNombreCabezaHogar', parent.document).val(objCabezaHogar.NombreCabezaHogar);

                                    $('#txtDocumentoCabezaHogar', parent.document).attr('disabled', 'disabled');
                                    $('#txtNombreCabezaHogar', parent.document).attr('disabled', 'disabled');
                                }
                                else {
                                    alerta('La persona cabeza de hogar no existe, por favor crearla en el modulo cabeza de hogar', 'Error');
                                    $('#txtDocumentoCabezaHogar', parent.document).val('');
                                    $('#txtDocumentoCabezaHogar', parent.document).focus();
                                }
                            }
                            catch (e) {
                                alerta('Ha ocurrido un error inesperado. Por favor contacte al administrador del sistema. ' + e.message, 'Error');
                ...
                parent.configChangeInput('txtFechaVisita', function () {
                    var Fecha = moment($('#txtFechaVisita', parent.document).val(), "DD/MM/YYYY");
                    ...
                    if (Fecha > fechaActual) {
                        ...
                        $('#txtFechaVisita', parent.document).val('');
                        $('#txtFechaVisita', parent.document).focus();
                    }
                ...
                parent.configFormValidate('EditarVisita', 'date, security');
